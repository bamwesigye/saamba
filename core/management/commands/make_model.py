...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        model, X_test, y_test = self.train_and_save_model(X, y, 'ftr_prediction_model')
        ...
        odds = df.loc[y_test.index, ['B365H', 'B365D', 'B365A']].to_dict('records')
    ...
    def calculate_roi(self, predictions, actual, odds, unit_stake, bank):
        ...
        for pred, act, match_odds in zip(predictions, actual, odds):
            if pred == act:
                wins += 1
                if pred == 'H' and not np.isnan(match_odds['B365H']):
                    returns += unit_stake * match_odds['B365H']
                elif pred == 'A' and not np.isnan(match_odds['B365A']):
                    returns += unit_stake * match_odds['B365A']
                elif pred == 'D' and not np.isnan(match_odds['B365D']):
                    returns += unit_stake * match_odds['B365D']
